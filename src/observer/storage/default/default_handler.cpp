...
RC DefaultHandler::create_index(Trx *trx, const char *dbname,
                                ...
                                const char *index_name, const int &attr_num,
    ...
    // 根据attr_num是否大于1，在这里进行分流一次 unique其实也可以用于multi-index
    // 因为不能让联合索引的key完全相同
    if (attr_num > 1) {
        return table->create_index(trx, index_name, attr_num, attribute_name,
                                   is_unique);
    }
...
RC DefaultHandler::insert_record(Trx *trx, const char *dbname,
                                 ...
                                 const Value *values, Record **record) {
    Table *table = find_table(dbname, relation_name);
    if (nullptr == table) {
